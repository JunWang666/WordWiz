...
namespace winrt::WordWiz::implementation
{
	...
	void DictionarySettingsPage::ImportSingleFileButton_Click(winrt::Windows::Foundation::IInspectable const& sender,
	...
	void DictionarySettingsPage::ImportMultipleFilesButton_Click(winrt::Windows::Foundation::IInspectable const& sender,
	                                                             winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
	{
	...
	winrt::fire_and_forget DictionarySettingsPage::ImportMultipleFilesAsync()
	{
		...
		try
		{
			...
			while (std::getline(iss, line))
			{
				...
				if (!line.empty())
				{
					strongThis->ShowMultipleFilesFlyout(L"正在验证文件路径...",
					                                    L"检查第 " + winrt::to_hstring(lineNumber) + L" 行: " +
					...
					if (!std::filesystem::exists(path))
					{
						strongThis->ShowMultipleFilesFlyout(
							L"文件不存在", L"第 " + winrt::to_hstring(lineNumber) + L" 行: " + winrt::to_hstring(line), false,
							false);
						co_await winrt::resume_after(std::chrono::seconds(3));
						strongThis->CloseFlyouts();
						co_return;
					...
					if (extension != ".db" && extension != ".sqlite")
					{
						strongThis->ShowMultipleFilesFlyout(L"不支持的文件格式",
						                                    L"第 " + winrt::to_hstring(lineNumber) + L" 行: " +
						                                    winrt::to_hstring(line) + L" (请选择 .db 或 .sqlite 文件)", false,
						                                    false);
						co_await winrt::resume_after(std::chrono::seconds(3));
						strongThis->CloseFlyouts();
						co_return;
			...
			strongThis->ShowMultipleFilesFlyout(L"开始导入词典文件...",
			                                    L"找到 " + winrt::to_hstring(filePaths.size()) + L" 个有效文件");
			strongThis->ImportFilesAsync(filePaths);
		}
		catch (const std::exception& ex)
		{
	...
	void DictionarySettingsPage::OpenDictionariesFolderButton_Click(
			...
			strongThis->DispatcherQueue().TryEnqueue([strongThis, results, filePaths]()
			{
				try
				{
					...
					else
					{
						...
						detailMessage = L"成功：" + winrt::to_hstring(successCount) + L"/" + winrt::to_hstring(totalCount)
							+ L" 个文件";
					}
	...
	void DictionarySettingsPage::ShowSingleFileFlyout(const winrt::hstring& message, bool showProgress, bool isSuccess)
	{
		try
		{
			...
			if (showProgress)
			{
				...
				auto blueBrush = Microsoft::UI::Xaml::Media::SolidColorBrush(
			...
			else
			{
				if (isSuccess)
				{
					...
					auto greenBrush = Microsoft::UI::Xaml::Media::SolidColorBrush(
						winrt::Windows::UI::ColorHelper::FromArgb(255, 16, 124, 16));
	...
	void DictionarySettingsPage::ShowMultipleFilesFlyout(const winrt::hstring& message, const winrt::hstring& detail,
