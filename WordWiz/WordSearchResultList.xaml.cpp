...
namespace winrt::WordWiz::implementation
{
	...
	Xaml::DependencyProperty WordSearchResultList::m_currentDetailItemProperty =
	...
	WordSearchResultList::WordSearchResultList()
	{
		...
		// 初始化 WordSearch 服务
		m_wordSearchService = winrt::make<WordSearch>();
	...
	void WordSearchResultList::CurrentDetailItem(const WordWiz::WordItem& value)
	{
		SetValue(m_currentDetailItemProperty, value);
	}

	// ListView 的 SelectionChanged 事件处理函数实现
	void WordSearchResultList::ResultsListView_SelectionChanged(const IInspectable& sender,
	...
	void WordSearchResultList::SearchTextBox_TextChanged(const Windows::Foundation::IInspectable& /*sender*/,
	                                                     const TextChangedEventArgs& /*args*/)
	{
		//m_debounceTimer.Stop(); // 每次输入变化时，重置计时器
		m_debounceTimer.Start();
	} // SearchTextBox 的 KeyDown 事件处理函数 (回车立即搜索)
	void WordSearchResultList::SearchTextBox_KeyDown(const Windows::Foundation::IInspectable& sender,
	...
	// 防抖计时器触发事件
	void WordSearchResultList::OnDebounceTimerTick(const Windows::Foundation::IInspectable& /*sender*/,
	...
	IAsyncAction WordSearchResultList::ExecuteSearchAsync(hstring query)
	{
		// 如果查询为空，显示无输入状态并清空列表
		if (query.empty())
		{
	...
	void WordSearchResultList::ShowNoInputState()
	{
		if (NoInputPanel())
		{
			NoInputPanel().Visibility(Visibility::Visible);
		}
		HideNoResultsState();
	}
	...
	void WordSearchResultList::HideNoInputState()
	{
		if (NoInputPanel())
		{
			NoInputPanel().Visibility(Visibility::Collapsed);
		}
	}
	...
	void WordSearchResultList::AddNewEntry(const hstring& word, const hstring& explanation)
	{
		auto newItem = winrt::make<WordItem>(word, explanation);
		m_items.Append(newItem);
	}

	void WordSearchResultList::searchWord(const hstring& query)
	{
