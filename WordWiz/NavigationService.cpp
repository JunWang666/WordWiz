...
// 静态成员变量定义
std::map<winrt::Microsoft::UI::Xaml::Controls::Frame, WordWizServices::NavigationService::FrameContext>
WordWizServices::NavigationService::s_frameContexts;
std::map<winrt::hstring, winrt::Windows::UI::Xaml::Interop::TypeName>
WordWizServices::NavigationService::s_knownPageTypesForNavView;
std::vector<WordWizServices::NavigationService::NavigatedListener>
WordWizServices::NavigationService::s_navigationListeners;
int64_t WordWizServices::NavigationService::s_nextTokenValue{1};

namespace WordWizServices
{
	// 辅助函数获取或创建 Frame 上下文
	NavigationService::FrameContext* NavigationService::GetFrameContext(
		const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame, bool createIfNotFound)
	{
	...
	void NavigationService::Initialize(
		...
		const winrt::Microsoft::UI::Xaml::Controls::NavigationView& associatedNavView)
	{
		if (!targetFrame)
		{
		...
		// 订阅 Frame 事件
		context->FrameNavigatedRevoker = targetFrame.
			Navigated(winrt::auto_revoke, &NavigationService::OnFrameNavigated);
		context->FrameNavigationFailedRevoker = targetFrame.NavigationFailed(
			winrt::auto_revoke, &NavigationService::OnFrameNavigationFailed);

		Log::LogMessage(L"NavigationService: Initialized for a Frame.");
	}

	void NavigationService::Uninitialize(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
	...
	bool NavigationService::IsInitialized(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
		...
		const winrt::Microsoft::UI::Xaml::Media::Animation::NavigationTransitionInfo& infoOverride)
	{
		...
		{
			Log::LogMessage(
				L"NavigationService: Navigating from tag to " + std::wstring(pageTypeNameString.c_str()) +
				L" in specified Frame.");
			return targetFrame.Navigate(it->second, parameter, infoOverride);
		}

		Log::LogMessage(
			L"NavigationService: PageTypeNameString not registered globally for NavView navigation: " + std::wstring(
				pageTypeNameString.c_str()));
		return false;
	}
		...
		const winrt::Microsoft::UI::Xaml::Media::Animation::NavigationTransitionInfo& infoOverride)
	{
		...
		{
			Log::LogMessage(
				L"NavigationService: CurrentPageType is invalid for specified Frame in AddCurrentPageToHistoryWithData.");
			return false;
		}

		Log::LogMessage(
			L"NavigationService: Adding current page to history with data in specified Frame - " + std::wstring(
				context->CurrentPageType.Name.c_str()));
		return targetFrame.Navigate(context->CurrentPageType, dataPacket, infoOverride);
	}

	bool NavigationService::GoBack(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame,
	...
	bool NavigationService::GoForward(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
	...
	bool NavigationService::CanGoBack(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
		return targetFrame && targetFrame.CanGoBack();
	}

	bool NavigationService::CanGoForward(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
		return targetFrame && targetFrame.CanGoForward();
	}

	void NavigationService::ClearNavigationHistory(const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
		if (targetFrame)
		{
	...
	winrt::Windows::UI::Xaml::Interop::TypeName NavigationService::GetCurrentPageType(
		const winrt::Microsoft::UI::Xaml::Controls::Frame& targetFrame)
	{
		FrameContext* context = GetFrameContext(targetFrame);
	...
	winrt::event_token NavigationService::AddNavigationListener(
		const std::function<void(const NavigationEventArgs&)>& listener)
	{
		winrt::event_token token{s_nextTokenValue++};
		s_navigationListeners.push_back({token, listener});
		Log::LogMessage(
			L"NavigationService: Added global navigation listener with token " + std::to_wstring(token.value));
		return token;
	}

	void NavigationService::RemoveNavigationListener(const winrt::event_token& token)
	{
		s_navigationListeners.erase(
			std::remove_if(s_navigationListeners.begin(), s_navigationListeners.end(),
			               [&](const NavigatedListener& listener_item) { return listener_item.token == token; }),
			s_navigationListeners.end());
		Log::LogMessage(
			L"NavigationService: Removed global navigation listener with token " + std::to_wstring(token.value));
	}
		...
		const winrt::Microsoft::UI::Xaml::Navigation::NavigationFailedEventArgs& e)
	{
		...
		if (failedFrame)
			ss << L" (Frame hash: " << std::hash<winrt::Windows::Foundation::IInspectable>{}(failedFrame)
				<< L")"; // 简单标识Frame
		...
		const winrt::Windows::Foundation::IInspectable& parameter)
	{
		...
		for (const auto& listener_item : s_navigationListeners)
		{
		...
		const winrt::Windows::UI::Xaml::Interop::TypeName& targetPageType)
	{
		...
		auto findAndSetItem = [&](const auto& items) -> bool
		{
			for (const auto& item : items)
			{
				if (auto navItem = item.template try_as<winrt::Microsoft::UI::Xaml::Controls::NavigationViewItem>())
				{
					if (auto tagInspectable = navItem.Tag())
					{
						if (auto tag = tagInspectable.template try_as<winrt::hstring>())
						{
		...
		{
			if (auto settingsItem = context->NavigationView.SettingsItem().try_as<
		...
		if (found)
		{
			...
			Log::LogMessage(
				L"NavigationService: Setting NavigationView selected item for " + std::wstring(
					targetPageTypeNameStr.c_str()) + L" associated with the Frame.");
		}
		else
		{
			context->NavigationView.SelectedItem(nullptr);
			Log::LogMessage(
				L"NavigationService: No matching NavigationViewItem found for " +
				std::wstring(targetPageTypeNameStr.c_str()) +
				L" in NavigationView associated with the Frame. Clearing selection.");
		}
	}
} // namespace WordWizServices
