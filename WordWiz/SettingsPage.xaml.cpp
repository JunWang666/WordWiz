...
namespace winrt::WordWiz::implementation
{
	...
	void SettingsPage::myButton_Click(const IInspectable&, const RoutedEventArgs&)
	{
		// 这个方法现在不再使用，因为我们已经移除了测试按钮
		// 保留空实现以避免编译错误
	}

	void SettingsPage::SettingsNavigationView_SelectionChanged(const IInspectable& sender,
	                                                           const NavigationViewSelectionChangedEventArgs& args)
	{
		if (auto selectedItem = args.SelectedItem())
		{
			...
			{
				auto tag = winrt::unbox_value_or<hstring>(navItem.Tag(), L"");
				NavigateToPage(tag);
			}
		}
	}

	void SettingsPage::NavigateToPage(const hstring& tag)
	{
		if (tag == L"General")
		{
