	...
	std::string DictionaryImporter::GetDictionaryIdFromDatabase(const std::string& filePath)
	{
		try
		{
			...
			try
			{
				...
				for (size_t i = 0; i < allRecords.rowCount() && i < 10; ++i)
				{
					// 最多显示10行
			...
			try
			{
				...
				if (dictionaryId.empty())
				{
					WordWizServices::Log::LogMessage(
						L"导入失败：未找到词典ID记录（AttributeName='ID'），拒绝导入: " + winrt::to_hstring(cleanFilePath));
					return "";
				}
		...
		{
			WordWizServices::Log::LogMessage(
				L"读取词典ID时发生异常: " + winrt::to_hstring(e.what()) + L" 文件: " + winrt::to_hstring(filePath));
			return "";
		}
		catch (...)
		{
