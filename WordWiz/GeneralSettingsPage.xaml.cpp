...
namespace winrt::WordWiz::implementation
{
	...
	void GeneralSettingsPage::ThemeRadio_Checked(const Windows::Foundation::IInspectable& sender,
	                                             const RoutedEventArgs& e)
	{
		auto radio = sender.as<RadioButton>();
		auto tag = winrt::unbox_value<hstring>(radio.Tag());
		std::string themeMode = to_string(tag);
	...
	bool GeneralSettingsPage::IsSystemInDarkMode()
	{
		try
		{
			...
			auto color = uiSettings.GetColorValue(Windows::UI::ViewManagement::UIColorType::Background);
	...
	void GeneralSettingsPage::LoadFolderPaths()
	{
		try
		{
			...
			{
				LocalFolderPathText().Text(to_hstring(localPath));
			...
			{
				LocalCacheFolderPathText().Text(to_hstring(cachePath));
			...
			try
			{
				...
				{
					SharedLocalFolderPathText().Text(to_hstring(sharedPath));
			...
			{
				TemporaryFolderPathText().Text(to_hstring(tempPath));
	...
	void GeneralSettingsPage::OpenFolderButton_Click(const Windows::Foundation::IInspectable& sender,
	                                                 const RoutedEventArgs& e)
	{
		auto button = sender.as<Button>();
		auto tag = winrt::unbox_value<hstring>(button.Tag());
		std::string folderType = to_string(tag);
		...
		try
		{
			...
			{
				folderPath = WordWizServices::Data::FilePathProvider::GetAppLocalFolderPath();
				// Settings is virtual, open parent folder
			}
	...
	void GeneralSettingsPage::OpenFolderInExplorer(const std::string& folderPath)
	{
		...
		{
			// 将路径转换为宽字符串
			hstring widePath = to_hstring(folderPath);

			// 使用 Windows::System::Launcher 打开文件夹
			auto uri = Uri(L"file:///" + widePath);
