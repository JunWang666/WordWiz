...
class ResourceEmbedder:
    def __init__(self, js_content_str=None, mdd_objects_list=None):
        ...
        if mdd_objects_list:
            for i, mdd_obj in enumerate(mdd_objects_list):
                try:
                    # Preload all items into a dictionary for faster lookup
                    content_dict = {key: value for key, value in mdd_obj.items()}

                    # Create a keys_cache for case-insensitive/path-style lookup
                    keys_cache_for_lookup = {
                        key.decode('utf-8', errors='replace').lower(): key
                        for key in content_dict.keys()
                    }

                    self.mdd_providers.append({
                        'obj': mdd_obj,
                        # Keep the MDD object reference if needed elsewhere (though not directly for lookup now)
                        'content_dict': content_dict,  # This is the preloaded content
                        'keys_cache': keys_cache_for_lookup,  # This is for finding the original byte key
                        'id': f"MDD_{i + 1}"
                    })
                except Exception as e:
                    print(f"警告：初始化 MDD 解析器 {i + 1} 的键缓存或加载内容时出错: {e}")
    ...
    def _find_mdd_resource(self, path_str_from_html):
        ...
        # Generate possible lowercased lookup keys for comparison
        possible_lookup_keys = [
            resource_name_cleaned.lower(),
            resource_name_cleaned.lstrip('/\\').lower(),
            ('/' + resource_name_cleaned.lstrip('/\\')).lower(),
            ('\\' + resource_name_cleaned.lstrip('/\\').replace('/', '\\')).lower()
        ]

        for provider in self.mdd_providers:
            content_dict = provider['content_dict']  # Now content_dict exists!
            keys_cache_for_lookup = provider['keys_cache']  # Use the keys_cache from init

            original_byte_key = None
            for lookup_key_decoded_lower in possible_lookup_keys:
                if lookup_key_decoded_lower in keys_cache_for_lookup:
                    original_byte_key = keys_cache_for_lookup[lookup_key_decoded_lower]
                    break  # Found the original byte key

            if original_byte_key and original_byte_key in content_dict:
                data_bytes = content_dict[original_byte_key]
                mime_type = get_mime_type(resource_name_cleaned)
                return data_bytes, mime_type, provider['id'], original_byte_key
            # else:
            # print(f"DEBUG: Resource for '{resource_name_cleaned}' (key: {original_byte_key!r}) not found in {provider['id']}.")
